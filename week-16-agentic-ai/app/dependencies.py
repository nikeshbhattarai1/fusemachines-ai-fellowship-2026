...
def get_agent_sources():
    """W16: documents (vector search) + structured facts registry, optionally wrapped with a fault injector."""
    global _agent_sources
    if _agent_sources is None:
        ...
        sources = AgentSources(get_retriever(), FactsRegistry(_settings.facts_path), SourceCatalog(_settings.manifest_path))
        spec = FaultSpec.parse(_settings.agent_fault_injection)
        _agent_sources = FaultyAgentSources(sources, spec) if spec else sources
    return _agent_sources

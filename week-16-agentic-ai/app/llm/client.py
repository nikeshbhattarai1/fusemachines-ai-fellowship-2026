...
class ProviderResponse:
    ...
    raw: Any = None
    # W16: provider-reported token usage, {"input_tokens": int, "output_tokens": int}.
    # None when the provider does not report it (the agent then estimates and flags it).
...
class AnthropicProvider(LLMProvider):
    ...
    def chat(self, messages, tools, temperature, top_p, max_tokens, tool_choice=None) -> ProviderResponse:
        ...
        return ProviderResponse(
            stop_reason=resp.stop_reason, text="".join(text_parts), tool_calls=tool_calls, raw=resp, usage=usage
        )
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    def chat(self, messages, tools, temperature, top_p, max_tokens, tool_choice=None) -> ProviderResponse:
        ...
        return ProviderResponse(
            stop_reason=stop_reason, text=msg.content or "", tool_calls=tool_calls, raw=resp, usage=usage
        )

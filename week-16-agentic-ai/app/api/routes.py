...
def ingest_corpus():
    """W16: load the demo verification corpus (data/sample_docs + data/corpus) into the vector store.

    Runs inside the API process on purpose: ChromaDB's persistent client must not be opened for writing
    by a second process (e.g. `docker compose exec`) while the API holds it. Idempotent.
    """
    from scripts.ingest_corpus import ensure_ingested

    return {"ingested": ensure_ingested(verbose=False)}
...
def verify(
    ...
    """W16: agentic cross-source verification loop.

    Not cached on purpose: results depend on tool outcomes (which can fail transiently) and on
    multi-turn clarification state, so replaying a stored answer could serve a degraded result.
    """

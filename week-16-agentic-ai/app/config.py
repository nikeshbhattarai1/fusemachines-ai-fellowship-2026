...
class Settings(BaseSettings):
    ...
    # W16 agentic verification loop
    agent_max_iterations: int = Field(default=8, alias="AGENT_MAX_ITERATIONS")
    agent_token_budget: int = Field(default=60000, alias="AGENT_TOKEN_BUDGET")
    agent_timeout_seconds: float = Field(default=90.0, alias="AGENT_TIMEOUT_SECONDS")
    agent_tool_timeout_seconds: float = Field(default=10.0, alias="AGENT_TOOL_TIMEOUT_SECONDS")
    agent_context_mode: str = Field(default="managed", alias="AGENT_CONTEXT_MODE")  # managed | raw (ablation)
    agent_ledger_max_entries: int = Field(default=20, alias="AGENT_LEDGER_MAX_ENTRIES")
    agent_snippet_chars: int = Field(default=1000, alias="AGENT_SNIPPET_CHARS")
    agent_min_score: float = Field(default=0.2, alias="AGENT_MIN_SCORE")
    # Dev/test only: e.g. "unavailable:facts_lookup", "malformed:kb_search", "timeout:kb_search"
    agent_fault_injection: Optional[str] = Field(default=None, alias="AGENT_FAULT_INJECTION")
    facts_path: str = Field(default="./data/facts/facts.json", alias="FACTS_PATH")
    manifest_path: str = Field(default="./data/corpus/manifest.json", alias="MANIFEST_PATH")
